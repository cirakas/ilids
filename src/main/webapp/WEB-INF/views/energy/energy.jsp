...
<script type="text/javascript">
    <%
        ...
        if (fromDt != null || admap != null || divId != null) {
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    //  System.out.println("-- " + cookie.getName());
                    if (cookie.getName().equals("amap")) {
                        cookie.setValue(admap);
                        response.addCookie(cookie);
                        System.out.println("-am- " + cookie.getValue());
                    }
                    if (cookie.getName().equals("did")) {
                        cookie.setValue(divId);
                        response.addCookie(cookie);
                        System.out.println("-dd- " + cookie.getValue());
                    }

                    if (cookie.getName().equals("fDate")) {
                        cookie.setValue(fromDt);
                        response.addCookie(cookie);
                        System.out.println("-fd- " + cookie.getValue());

                    }

                    if (cookie.getName().equals("fHour")) {
                        cookie.setValue(fromHr);
                        response.addCookie(cookie);
                        System.out.println("-fh- " + cookie.getValue());
                    }

                    if (cookie.getName().equals("fMin")) {
                        cookie.setValue(fromMin);
                        response.addCookie(cookie);
                        System.out.println("-fm- " + cookie.getValue());
                    }

                    if (cookie.getName().equals("toDt")) {
                        cookie.setValue(toDt);
                        response.addCookie(cookie);
                        System.out.println("-td- " + cookie.getValue());
                    }

                    if (cookie.getName().equals("toHr")) {
                        cookie.setValue(toHr);
                        response.addCookie(cookie);
                        System.out.println("-th- " + cookie.getValue());
                    }

                    if (cookie.getName().equals("toMin")) {
                        cookie.setValue(toMin);
                        response.addCookie(cookie);
                        System.out.println("-tm- " + cookie.getValue());
                    }
                }
            } 
            
            else {   //set cookie
                Cookie amapCook = new Cookie("amap", admap);
                response.addCookie(amapCook);
                amapCook.setMaxAge(60 * 60 * 24); //1 hour

                Cookie deviceCook = new Cookie("did", divId);
                response.addCookie(deviceCook);
                deviceCook.setMaxAge(60 * 60);

                Cookie frDateCook = new Cookie("fDate", fromDt);
                response.addCookie(frDateCook);
                frDateCook.setMaxAge(60 * 60); //1 hour

                Cookie frHrCook = new Cookie("fHour", fromHr);
                response.addCookie(frHrCook);
                frHrCook.setMaxAge(60 * 60);

                Cookie frMinCook = new Cookie("fMin", fromMin);
                response.addCookie(frMinCook);
                frMinCook.setMaxAge(60 * 60); //1 hour

                Cookie toDateCook = new Cookie("toDt", toDt);
                response.addCookie(toDateCook);
                toDateCook.setMaxAge(60 * 60);

                Cookie toHrCook = new Cookie("toHr", toHr);
                response.addCookie(toHrCook);
                toHrCook.setMaxAge(60 * 60); //1 hour

                Cookie toMinCook = new Cookie("toMin", toMin);
                response.addCookie(toMinCook);
                toMinCook.setMaxAge(60 * 60);
                System.out.println("dfdg----" + amapCook.getValue());
            }
        }
...
//    })();
    ...
    var fromDateParam = "<%=fromDt%>";
    var toDateParam = "<%=toDt%>";
    var fromHoursParam = "<%=fromHr%>";
    var fromMinutesParam = "<%=fromMin%>";
    var toHoursParam = "<%=toHr%>";
    var toMinutesParam = "<%=toMin%>";
    var deviceParam = "<%=divId%>";
...
<div class="row" style="margin-bottom: 0;padding-left: 235px;">
    <div class="col-lg-6 graph_bg" style="">
        <div class="panel panel-primary panel_1" style="width : 100%; ">
