...
NVALUE neg_val;
                    ...
                    for(i=3;i<(pktcount-2);)
                    {
                        ...
                        tcval=0.0;
                        if(!compare_float(params1rand[db_id1].val,0.0,params1rand[db_id1].offset))//do not generate if previous val is 0,this means there is no range available for this param
                        {
                            tcval=(params1rand[db_id1].val-RANGEVAL) + (drand48() * ((params1rand[db_id1].val+RANGEVAL) - (params1rand[db_id1].val-RANGEVAL)));
                            if((params1rand[db_id1].addr_off==30)||(params1rand[db_id1].addr_off==32)||(params1rand[db_id1].addr_off==34))//for power factor
                            {
                                tcval=drand48();
                            }
                        }
